...
回答门店试坐、试乘或试驾问题时，先区分静态试坐与动态试驾：静态检查着地、骑姿、原地扶正和挪车感受；动态试驾必须以已取得相应驾驶资格、佩戴合格护具、遵守门店规则和在合法安全路线内为前提，再提醒低速操控、制动、动力响应和异常声响。不要鼓励无证试驾，也不要把短暂试驾结论说成真实长期车况。
回答门店、销售或购车交易风险时，先给直接判断，例如“会不会坑不能先下结论，但这几处最容易藏费用”。重点提醒用户索要同年款同配置的书面报价，拆清裸车、保险、上牌、金融服务和其他费用；核对定金/订金、退款条件、交付配置与赠品是否写入合同；付款前检查车辆信息并保留凭证。语气像朋友提醒关键坑位，不要写成法规宣讲或泛泛的安全手册，也不要因此重新推荐车型。
...
只返回 JSON：{"answer":"...","redirect_suggestions":["开始新手选车","对比两款车","核查二手车源"]}`;
  ...
  function fallback(mode, errorCode = null, rawText = "") {
    ...
    const testRide = /试乘|试驾|试坐|门店看车|到店看车|到店验车|原地挪车/.test(rawText);
    const dealerRisk = /(门店|车行|销售|经销商).*(坑|套路|注意|报价|费用|合同|订金|定金|赠品|加价|落地)|(?:坑|套路).*(门店|车行|销售|经销商)/.test(rawText);
    return {
      status: "fallback",
      answer: testRide
        ? "先静态试坐：看双脚着地、骑姿是否别扭，再试试扶正车身、原地倒车和小半径挪车。动态试驾前要确认自己有相应驾驶资格，按门店要求戴好护具并走合法安全路线；重点感受低速控制、制动手感、动力响应和有无异常声响。短时间试驾只能判断基础适配，价格、配置和车况还要另行核对。"
        : dealerRisk
        ? "会不会坑现在不能先下结论，但有几处最容易藏费用。让销售把同年款、同配置的报价写清楚，裸车、保险、上牌、金融服务和其他费用逐项拆开；口头承诺的赠品、交付时间和退款条件也要进合同。尤其看清写的是“定金”还是“订金”，别在费用和退款规则没确认前急着付款。"
        : beginnerSafety
