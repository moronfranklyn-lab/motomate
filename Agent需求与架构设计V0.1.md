...
- 会话与隐私规则：默认携带结构化记忆、最近 6 条消息和当前问题；完整文字会话仅在授权后保存 30 天；连续 180 天未登录自动清理保存记录；二手原图当次分析完成或终止后删除。
...
| Need Extractor | 抽取购车需求字段 | 文本、结构化记忆、最近 6 条消息 | `UserNeed` |
...
- 每次模型调用默认携带：结构化用户记忆、最近 6 条消息、当前问题。
...
- 用户明确表达可写入当前会话的临时结构化状态；不得因此自动形成跨会话长期记录。
- AI 推断必须经用户确认。
- 每条记忆保存来源、更新时间、确认类型。
- 只有用户主动保存后，确认字段才写入账户中的长期购车记录。
...
- 查看完整原文必须满足：用户已单独授权、仍在 30 天期限内、管理员填写复盘原因，并记录查看人、时间、会话 ID。
- 到期或用户删除后不得继续访问。
